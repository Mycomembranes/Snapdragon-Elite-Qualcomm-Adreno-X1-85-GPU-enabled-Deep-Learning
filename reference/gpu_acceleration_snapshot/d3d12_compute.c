...
uint64_t d3d12c_create_compute_pipeline(
    const uint32_t *spirv, uint32_t spirv_words,
    uint32_t num_uavs, uint32_t num_cbvs)
{
    ...
    conf.shader_model_max = SHADER_MODEL_6_2_;
    ...
    /* Create root signature: N UAVs + M CBVs as root descriptors */
    uint32_t total_params = num_uavs + num_cbvs;
    D3D12_ROOT_PARAMETER *params = NULL;
    if (total_params > 0) {
        params = (D3D12_ROOT_PARAMETER*)calloc(total_params, sizeof(D3D12_ROOT_PARAMETER));
        ...
        for (uint32_t i = 0; i < num_cbvs; i++) {
            params[num_uavs + i].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
            params[num_uavs + i].Descriptor.ShaderRegister = i;
            params[num_uavs + i].Descriptor.RegisterSpace = 0;
            params[num_uavs + i].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;
        }
    }

    D3D12_ROOT_SIGNATURE_DESC rs_desc = {0};
    rs_desc.NumParameters = total_params;
    ...
    if (FAILED(hr)) {
        if (error_blob) {
            set_error("SerializeRootSignature failed: %s",
                (char*)ID3D10Blob_GetBufferPointer(error_blob));
            ID3D10Blob_Release(error_blob);
        } else {
            set_error("SerializeRootSignature failed: 0x%08x", (unsigned)hr);
        }
    ...
    if (FAILED(hr)) {
        set_error("CreateRootSignature failed: 0x%08x", (unsigned)hr);
        g.pfn_spirv_to_dxil_free(&dxil_obj);
        return 0;
    }
    ...
    g.handles[h - 1].pipeline.num_uavs = num_uavs;
    g.handles[h - 1].pipeline.num_cbvs = num_cbvs;
    return h;
}
    ...
    uint32_t groups_x, uint32_t groups_y, uint32_t groups_z)
{
    ...
    for (uint32_t i = 0; i < num_uavs; i++) {
        ...
        D3D12_GPU_VIRTUAL_ADDRESS addr = ID3D12Resource_GetGPUVirtualAddress(be->resource);
        ID3D12GraphicsCommandList_SetComputeRootUnorderedAccessView(g.cmdlist, i, addr);
    }

    /* Bind CBV buffers as root CBVs */
    for (uint32_t i = 0; i < num_cbvs; i++) {
        ...
        D3D12_GPU_VIRTUAL_ADDRESS addr = ID3D12Resource_GetGPUVirtualAddress(be->resource);
        ID3D12GraphicsCommandList_SetComputeRootConstantBufferView(g.cmdlist, num_uavs + i, addr);
    }
    ...
    uint32_t groups_x, uint32_t groups_y, uint32_t groups_z)
{
    ...
    if (d3d12c_dispatch(pipeline_handle, uav_handles, num_uavs,

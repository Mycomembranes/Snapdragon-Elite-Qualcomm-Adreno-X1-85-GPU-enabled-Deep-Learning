...
def _get_or_create_pipeline(key, spirv_bytes, num_uavs, num_cbvs):
    ...
    # Convert SPIR-V bytes to uint32 array
    if len(spirv_bytes) % 4 != 0:
        raise ValueError("SPIR-V binary size must be multiple of 4")
    spirv_words = len(spirv_bytes) // 4
    spirv_arr = (ctypes.c_uint32 * spirv_words).from_buffer_copy(spirv_bytes)

    handle = lib.d3d12c_create_compute_pipeline(
        ctypes.cast(spirv_arr, ctypes.c_void_p),
        spirv_words, num_uavs, num_cbvs
    )
    if handle == 0:
        err = lib.d3d12c_get_last_error()
...
def _dispatch_compute(pipeline_handle, uav_handles, cbv_handles,
                      groups_x, groups_y=1, groups_z=1):
    """Execute a compute dispatch with given pipeline and buffer bindings."""
    lib = _load_library()
...
def _dispatch_batch(dispatches):
    ...
    dispatches: list of (pipeline_handle, uav_handles, cbv_handles, gx, gy, gz)
    """
    lib = _load_library()
    ...
    for pipeline_handle, uav_handles, cbv_handles, gx, gy, gz in dispatches:
        num_uavs = len(uav_handles)
        num_cbvs = len(cbv_handles)
...
def _ensure_pipeline(name, spirv_bytes, num_uavs, num_cbvs):
    """Get or create a cached pipeline from SPIR-V bytes."""
    return _get_or_create_pipeline(name, spirv_bytes, num_uavs, num_cbvs)
...
def d3d12_add(a, b):
    ...
    spv = _load_spirv("add")
    pipeline = _ensure_pipeline("add", spv, 3, 0)  # 3 UAVs: a, b, out
...
def d3d12_sub(a, b):
    ...
    spv = _load_spirv("sub")
    pipeline = _ensure_pipeline("sub", spv, 3, 0)
    out = D3D12Tensor.empty(a.shape)
    n = int(np.prod(a.shape))
    groups = (n + 255) // 256
    _dispatch_compute(pipeline, [a.handle, b.handle, out.handle], [], groups)
...
def d3d12_mul(a, b):
    ...
    assert a.shape == b.shape
    spv = _load_spirv("mul")
    pipeline = _ensure_pipeline("mul", spv, 3, 0)
    out = D3D12Tensor.empty(a.shape)
    n = int(np.prod(a.shape))
    groups = (n + 255) // 256
    _dispatch_compute(pipeline, [a.handle, b.handle, out.handle], [], groups)
...
def d3d12_scalar_mul(a, scalar):
    ...
    pipeline = _ensure_pipeline("scalar_mul", spv, 2, 1)  # 2 UAVs + 1 CBV
    out = D3D12Tensor.empty(a.shape)
...
def d3d12_relu(x):
    ...
    pipeline = _ensure_pipeline("relu", spv, 2, 0)
...
def d3d12_gelu(x):
    ...
    pipeline = _ensure_pipeline("gelu", spv, 2, 0)
    out = D3D12Tensor.empty(x.shape)
    n = int(np.prod(x.shape))
    groups = (n + 255) // 256
    _dispatch_compute(pipeline, [x.handle, out.handle], [], groups)
...
def d3d12_sigmoid(x):
    ...
    pipeline = _ensure_pipeline("sigmoid", spv, 2, 0)
    out = D3D12Tensor.empty(x.shape)
    n = int(np.prod(x.shape))
    groups = (n + 255) // 256
    _dispatch_compute(pipeline, [x.handle, out.handle], [], groups)
...
def d3d12_tanh(x):
    ...
    pipeline = _ensure_pipeline("tanh_act", spv, 2, 0)
    out = D3D12Tensor.empty(x.shape)
    n = int(np.prod(x.shape))
    groups = (n + 255) // 256
    _dispatch_compute(pipeline, [x.handle, out.handle], [], groups)
...
def d3d12_matmul(a, b):
    ...
    pipeline = _ensure_pipeline("matmul", spv, 3, 1)  # a, b, out + params
...
def d3d12_transpose(x):
    ...
    pipeline = _ensure_pipeline("transpose_2d", spv, 2, 1)
...
def d3d12_matmul_add_relu(a, b, bias):
    ...
    # a, b, bias, out, pre_relu + params
    pipeline = _ensure_pipeline("matmul_add_relu", spv, 5, 1)
    ...
    _dispatch_compute(pipeline,
                      [a.handle, b.handle, bias.handle, out.handle, pre_relu.handle],
                      [params.handle],
                      groups_x, groups_y, 1)
    return out, pre_relu
...
def d3d12_matmul_add(a, b, bias):
    ...
    pipeline = _ensure_pipeline("matmul_add", spv, 4, 1)
    ...
    _dispatch_compute(pipeline,
                      [a.handle, b.handle, bias.handle, out.handle],
                      [params.handle],
                      groups_x, groups_y, 1)
    return out
...
def d3d12_relu_backward(grad_out, x):
    ...
    pipeline = _ensure_pipeline("relu_backward", spv, 3, 0)
...
def d3d12_gelu_backward(grad_out, x):
    ...
    pipeline = _ensure_pipeline("gelu_backward", spv, 3, 0)
    grad_in = D3D12Tensor.empty(x.shape)
    n = int(np.prod(x.shape))
    groups = (n + 255) // 256
    _dispatch_compute(pipeline, [grad_out.handle, x.handle, grad_in.handle], [], groups)
...
def d3d12_sum(x):
    ...
    while current_n > 1:
        ...
        pipeline = _ensure_pipeline("sum_reduce", spv, 2, 1)
        groups = out_n
...
def d3d12_mean(x):
    ...
    pipeline = _ensure_pipeline("mean_reduce", spv, 2, 1)
    n = int(np.prod(x.shape))
    out_n = (n + 255) // 256
    out = D3D12Tensor.empty((out_n,))
...
def d3d12_layer_norm(x, gamma, beta, eps=1e-5):
    ...
    pipeline = _ensure_pipeline("layer_norm", spv, 4, 1)
    out = D3D12Tensor.empty(x.shape)
...
def d3d12_softmax(x):
    ...
    pipeline = _ensure_pipeline("softmax", spv, 2, 1)
    out = D3D12Tensor.empty(x.shape)
